...
import { ExternalLink, X, Eye, Heart, Repeat2, MousePointerClick } from "lucide-react";

const THEME_STYLES: Record<
  string,
  { border: string; glow: string; accent: string; bg: string; emoji: string }
> = {
...
function VisualCard({ post }: { post: SocialPost }) {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center rounded-2xl overflow-hidden"
      ...
      <div className="absolute bottom-0 left-0 right-0 h-px"
        style={{ background: `linear-gradient(90deg, transparent, ${t.border}66, transparent)` }} />
    </div>
  );
}
...
export default function SocialPostCard({ post, tweetMetrics, igMetrics, tiktokMetrics }: Props) {
  const t = THEME_STYLES[post.visualTheme] ?? THEME_STYLES.pulse;
  ...
  return (
    <article className="rounded-2xl border border-white/[0.07] bg-[#161c28] overflow-hidden">
      <div className="p-4"><VisualCard post={post} /></div>
